...
export default function SearchPage() {
  ...
  // Location filter expansion state (matches student version)
    ...
    rows.reduce((acc, { prefecture, city }) => {
      ...
      if (city) {
        if (!acc[region][prefecture]) {
          acc[region][prefecture] = new Set<string>()
        }
        acc[region][prefecture].add(city)
      }
      return acc
    }, {} as Record<string, Record<string, Set<string>>>)
  ).map(([name, prefs]) => ({
    name,
    prefectures: Object.entries(prefs).map(([prefName, citySet]) => ({
      name: prefName,
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault()
    // You can customize search behavior here, for now it only prevents default form submission
  }
  // 時給レンジ
  ...
  return (
    <>
      {/* Header */}
      <header className="px-4 lg:px-6 h-16 flex items-center justify-center border-b bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/60 sticky top-0 z-50">
